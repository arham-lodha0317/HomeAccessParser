...
loginInfo = {
    ...
    "LogOnDetails.UserName": "aarya.oswal",
    "tempUN": "aarya.oswal",
    "tempPW": "12132004",
    "LogOnDetails.Password": "12132004",
    "login": ""
}

getMarkingPeriod = {
    "ctl00$plnMain$ddlReportCardRuns": "3-2020",
...
loginurl = "https://hac.friscoisd.org/HomeAccess/Account/LogOn?ReturnUrl=%2fhomeaccess%2f"
getUrl = "https://hac.friscoisd.org/HomeAccess/Content/Student/Assignments.aspx"
...
with requests.Session() as session:
    ...
    get = session.get(getUrl)
    p1 = BeautifulSoup(get.text, 'html.parser')

    num = 0

    # get grades
    for div in p1.find_all('div', class_="AssignmentClass"):
        classes = div.findChild("a", class_="sg-header-heading").text.strip()
        average = div.findChild(
            "span", class_="sg-header-heading").text.strip()[15:]

        l1 = div.findChildren("tr", class_="sg-asp-table-data-row")

        assignments = []
        for num in range(len(l1)-2):
            i = l1[num]
            assignment = i.findChild("a").text.strip()
            typeOFGrade = i.contents[4].text.strip()
            number = i.contents[10].text.strip()
            grade = {
                assignment: number,
                "type": typeOFGrade
            }
